...
function post_flagger_admin_init() {
	...
	wp_register_style( 'myPluginStylesheet', plugins_url( 'styles.css', __FILE__ ) );
}
		...
		if ( isset( $_GET['action'] ) ) :
			...
			switch ( $_GET['action'] ) {
				...
				case 'delete':
					...
					wp_redirect( admin_url( 'options-general.php?page=post-flagger-options&m=2' ) );
					break;
			}
			...
			// Show title
			echo '<h2>Post Flags' . pf_add_new_button() . '</h2>';
			// Show my WP_List_Table
			$testListTable->display();

			//Show add new button
			echo pf_add_new_button('New Flag', 'button button-primary button-large');
		endif;
